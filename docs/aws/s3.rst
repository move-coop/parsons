...
The :class:`~s3.S3` class allows interaction with Amazon Web Service's `object storage service <https://aws.amazon.com/s3/>`_
...
them to be accepted. The :class:`~s3.S3` class can be passed a session token as an environmental variable (``AWS_SESSION_TOKEN``) or as a keyword argument.
