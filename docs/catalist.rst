...
The CatalistMatch class allows you to interact with the Catalist M Tool (match) API.
Users of this Parsons integration can use the Parsons table format to send input
files to the M Tool and receive back a matched version of that table.

.. admonition:: Authentication

   In order to use this class you must be provided with an
...
``client_id``, ``client_secret``, ``sftp_username`` and ``sftp_password`` values as arguments:
