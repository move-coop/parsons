...
* Classes that call directly into HTTP API's using the Parsons :class:`~parsons.utilities.api_connector.APIConnector` class
...
By using mocks in our tests, we make it easier to set up and run our tests without having to worry about having the proper credentials, etc.
...
For classes built using the :class:`~parsons.utilities.api_connector.APIConnector` class, we can leverage the ``requests-mock`` library, since the :class:`~parsons.utilities.api_connector.APIConnector` uses the ``requests`` library.
...
For an example of testing a class built on the :class:`~parsons.utilities.api_connector.APIConnector`, we can look at the :class:`~parsons.mailchimp.mailchimp.Mailchimp` connector.

In the code below, we have our ``TestMailchimp`` that serves as our test case. We have one test method to test calls to our
:meth:`~parsons.mailchimp.mailchimp.Mailchimp.get_campaigns` method in our :class:`~parsons.mailchimp.mailchimp.Mailchimp` Connector class.
...
To configure our mock API, we can make use of the ``get``, ``post``, ``patch``, ``put``, and ``delete`` methods on the mock object ``m``.
These methods mirror the calls we expect our Connector to make to the API.
The methods on the mock object correspond to the HTTP method that we are expecting our Connector class to call with,
and the first argument to the mock method is the URL that we expect our Connector to call.
We can then provide a ``json`` argument to the mock method to provide a canned response that the mock HTTP server should return.

Below, we will use the ``get`` method to tell the mock HTTP server that we expect a call to the ``campaigns`` endpoint of our API,
...
After wiring up the mock HTTP API, we are ready to call the :meth:`~parsons.mailchimp.mailchimp.Mailchimp.get_campaigns` method on our :class:`~parsons.mailchimp.mailchimp.Mailchimp` connector.
...
which can help keep the test code separate from the test data, which helps makes tests more readable.
The test data is imported from the ``expected_json.py`` Python file that sits alongside the ``test_mailchimp.py`` test file.
...
For classes that wrap third party libraries, we are looking to simulate whatever client object that serves as the Python API for the calls to the external service.
To simulate the client object, we can use the ``unittest.mock.MagicMock`` class from the Python standard library.

The ``MagicMock`` class allows us to build a fake version of the client object by simulating functionality we expect our
Connector class to use -- namely by returning canned responses to method calls on the client.
As well, the ``MagicMock`` class is recording those calls as they happen, so that after we have run through our tests,
we can check that things happened in the way we expected.

The following gives a simple example of initializing, setting up, and checking expectations on a ``MagicMock`` class.
...
The :class:`~parsons.salesforce.salesforce.Salesforce` class is a good example for writing tests for Connector classes written against a third party library.
The :class:`~parsons.salesforce.salesforce.Salesforce` Parsons Connector class wraps around the ``simple-salesforce`` library's Salesforce client.
When testing the :class:`~parsons.salesforce.salesforce.Salesforce` Parsons class, we will need to swap out its reference to the ``simple-salesforce`` client with a mock client.
...
By overriding it with our ``MagicMock`` object, the :class:`~parsons.salesforce.salesforce.Salesforce` Parsons class will be calling methods on our mock client instead of an actual simple-salesforce client.
...
Finally, we test to make sure that our :class:`~parsons.salesforce.salesforce.Salesforce` Connector returned the expected response,
