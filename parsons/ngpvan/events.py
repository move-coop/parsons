...
class Events:
    ...
    def update_event(self, event_id, name=None, short_name=None, start_date=None, end_date=None,
                     event_type_id=None, roles=None, shifts=None, description=None,
                     location_ids=None, code_ids=None, notes=None):
        """
        Update an event
        ...
        # event object, update it and then return it as a "PUT" request.

        # Get the existing event json object
        ...
        # Populate the more complex values if not null.
        if location_ids:
            event['locations'] = [{'locationId': l} for l in location_ids],
        ...
        if event_type_id:
            event['eventType'] = {'eventTypeId': event_type_id}

        if shifts:
            event['shifts'] = [{'name': s['name'],
                                'startTime': s['start_time'],
                                'endTime': s['end_time']} for s in shifts]

        self.connection.put_request(f'events/{event_id}', json=event)
    ...
    def get_event_types(self):
        ...
        logger.info(f'Found {tbl.num_rows} event types.')
        return tbl
