...
class CountyDetails:
    ...
    A dataclass is decorator that adds special functions including an automatic __init__ function.
    See more here: https://docs.python.org/3/library/dataclasses.html
    """
...
class Scytl:
    ...
    def _parse_date_to_utc(self, input_dt: str) -> datetime | None:
        ...
        Args:
            input_dt (str): The datetime string to be parsed.
    ...
    def _get_version(self, administrator: str, election_id: str) -> str:
        ...
        Args:
            administrator (str): The url code for the election administrator, either the two-letter state code or
                the state code and the county, separated by a slash.
            election_id (str): The election id for the given election as a string.

        Returns:
            str: The version id as a string.
    ...
    def _parse_file_from_zip_url(self, zipfile_url: str, file_name: str) -> bytes:
        ...
        Args:
            zipfile_url (str): The url where the zip file can be found.
            file_name (str): The expected name of the file in the zipfile to read.

        Returns:
            bytes: The unzipped file as bytes.
    ...
    def _get_latest_counties_scytl_info(
        ...
        """
        Fetch the settings JSON file for the election and parse the county details for participating counties in a state
        election.

        Args:
            state (str): The two-letter state code for the state.
            election_id (str): The election ID for the given election.
            version_num (str): The latest version ID of the election as a string.

        Returns:
            dict[str, CountyDetails]: A dictionary mapping county names to their sub-election information.
    ...
    def _parse_county_xml_data_to_precincts(
        ...
        """
        Parse a detail XML file for a county into a list of election results by precinct and vote method.

        Args:
            county_data (bytes): Bytes The detail XML file for a county as bytes.
            county_details (CountyDetails): The details class for the county, including name, id, and last updated
                datetime.

        Returns:
            list[dict]: The list of election results by precinct and vote method in the file.
    ...
    def _parse_state_xml_data_to_counties(self, state_data: bytes, state: str) -> list[dict]:
        """
        Parse a detail XML file for a state into a list of election results by county and vote method.

        Args:
            state_data (bytes): Bytes The detail XML file for a state as bytes.
            state (str): The two-letter state code for the state associated with the file.

        Returns:
            list[dict]: The list of election results by state and vote method in the file.
    ...
    def _fetch_and_parse_summary_results(
        ...
        """
        Fetches the summary results CSV file from the Scytl site and parses it into a list of election results by
        candidate.
        ...
        Args:
            ...
            election_id (str): The election id for the given election as a string.
            version_num (str): The latest version ID of the election as a string.
            county (str, optional): The name of the county associated with the summary file.
                Defaults to "".

        Returns:
            list[dict]: The list of election results by candidate.
    ...
    def get_detailed_results(self, force_update: bool = False) -> list[dict] | None:
        """
        ...
        Returns:
            list[dict]: Entries for each candidate in each office, per vote method and county.
                ...
                **If fetching for a state, results are**
                ...
                - precincts_reporting
                - total_precincts
                - vote_method (note: some administrators choose to differentiate results by vote method, while others do
                not)
                - candidate_name
                ...
                **If fetching for a county, results are**
    ...
    def get_detailed_results_for_participating_counties(
        ...
        """
        Fetch the latest detailed results for the given election for all participating counties with detailed results,
        across all contests.

        Some counties may not have detailed results. If so, this will attempt to fetch the summary results for that
        county. If no results exist for either, the county name will be appended to the missing_counties list.

        After the first fetch, only the counties with updates will be returned, previous results will not be included.

        Please note that all electoral entities administer their elections differently, so not all values will be
        populated if the entity doesn't provide them.

        Args:
            county_names (list[str], optional): List[str] The list of counties to get precinct-level results for.
        ...
        Returns:
            tuple[list[str], list[dict]]: A tuple containing:
                - missing_counties (list[str]): A list of county names where data could not be fetched (no detailed or
                summary results found).
                - parsed_data (list[dict]): A list of dictionaries representing results per candidate, office, vote
                method, and precinct.
