"""Connector class for interacting with the SolidarityTech Event Sessions endpoint."""
...
class SolidarityTechEventSessions(SolidarityTechBase):
    ...
    def get_event_sessions(
        ...
        *,
        ...
        Returns:
            A list of event session entries, along with requested metadata.
        ...
        _ = upcoming is not None and params.update({"upcoming": upcoming})
        _ = starts_after is not None and params.update({"starts_after": starts_after})
        _ = starts_before is not None and params.update({"starts_before": starts_before})
        _ = chapter_id is not None and params.update({"chapter_id": chapter_id})
        _ = event_tags is not None and params.update({"event_tags": event_tags})
        _ = include_rsvp_counts is not None and params.update(
            {"include_rsvp_counts": include_rsvp_counts}
        )
        _ = include_confirmed_counts is not None and params.update(
            {"include_confirmed_counts": include_confirmed_counts}
        )
        _ = include_hosts is not None and params.update({"include_hosts": include_hosts})
        _ = count is not None and params.update({"count": count if count else None})
    ...
    def create_event_session(
        ...
        *,
        ...
        Returns:
            A single event session entry, as it exists after creating.
        ...
        _ = event_type is not None and payload.update({"event_type": event_type})
        _ = location_name is not None and payload.update({"location_name": location_name})
        _ = location_data is not None and payload.update({"location_data": location_data})
        _ = location_address is not None and payload.update({"location_address": location_address})
        _ = show_rsvp_bar is not None and payload.update({"show_rsvp_bar": show_rsvp_bar})
        _ = show_title_in_form is not None and payload.update(
            {"show_title_in_form": show_title_in_form}
        )
        _ = note is not None and payload.update({"note": note})
    ...
    def get_event_session(
        ...
        *,
        ...
        Returns:
            A single event session entry, along with requested metadata.
    ...
    def update_event_session(
        ...
        *,
        ...
        Returns:
            A single event session entry, as it exists after updating.
        ...
        """
        payload: dict[str, _JsonType] = {}
        _ = start_time is not None and payload.update({"start_time": start_time})
        _ = end_time is not None and payload.update({"end_time": end_time})
        _ = title is not None and payload.update({"title": title})
        _ = location_name is not None and payload.update({"location_name": location_name})
        _ = location_address is not None and payload.update({"location_address": location_address})
        _ = location_data is not None and payload.update({"location_data": location_data})
        _ = show_rsvp_bar is not None and payload.update({"show_rsvp_bar": show_rsvp_bar})
    ...
    def add_event_host(
        ...
        Returns:
            A single event session entry, as it exists after adding the host.
    ...
    def remove_event_host(
        ...
        Returns:
            A single event session entry, as it exists after removing the host.
