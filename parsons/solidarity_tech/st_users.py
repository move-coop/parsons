"""Connector class for interacting with the SolidarityTech Users endpoint."""
...
class SolidarityTechUsers(SolidarityTechBase):
    ...
    def get_users(
        ...
        _ = user_list_ids is not None and params.update({"user_list_ids": user_list_ids})
        _ = phone_number is not None and params.update({"phone_number": phone_number})
        _ = email is not None and params.update({"email": email})
    ...
    def create_user(
        ...
        *,
        ...
        _ = phone_number is not None and payload.update({"phone_number": phone_number})
        _ = email is not None and payload.update({"email": email})
        _ = first_name is not None and payload.update({"first_name": first_name})
        _ = last_name is not None and payload.update({"last_name": last_name})
        _ = alternate_name is not None and payload.update({"alternate_name": alternate_name})
        _ = preferred_language is not None and payload.update(
            {"preferred_language": preferred_language}
        )
        _ = second_language is not None and payload.update({"second_language": second_language})
        _ = chapter_id is not None and payload.update({"chapter_id": chapter_id})
        _ = chapter_ids is not None and payload.update({"chapter_ids": chapter_ids})
        _ = referred_by_user_id is not None and payload.update(
            {"referred_by_user_id": referred_by_user_id}
        )
        _ = custom_user_properties is not None and payload.update(
            {"custom_user_properties": custom_user_properties}
        )
        _ = add_tags is not None and payload.update({"add_tags": add_tags})
        _ = remove_tags is not None and payload.update({"remove_tags": remove_tags})
        _ = donation_charge is not None and payload.update({"donation_charge": donation_charge})
        _ = address is not None and payload.update({"address": address})
        _ = assessment is not None and payload.update({"assessment": assessment})
        _ = sms_permission is not None and payload.update({"sms_permission": sms_permission})
        _ = call_permission is not None and payload.update({"call_permission": call_permission})
        _ = email_permission is not None and payload.update({"email_permission": email_permission})
        _ = timezone is not None and payload.update({"timezone": timezone})
        _ = lookup_key is not None and payload.update({"lookup_key": lookup_key})
    ...
    def update_user(
        ...
        *,
        ...
        _ = phone_number is not None and payload.update({"phone_number": phone_number})
        _ = clear_phone_number is not None and payload.update(
        ...
        _ = chapter_id is not None and payload.update({"chapter_id": chapter_id})
        _ = chapter_ids is not None and payload.update({"chapter_ids": chapter_ids})
        _ = add_chapter_ids is not None and payload.update({"add_chapter_ids": add_chapter_ids})
        _ = remove_chapter_ids is not None and payload.update(
            {"remove_chapter_ids": remove_chapter_ids}
        )
        _ = set_exclusive_chapter is not None and payload.update(
