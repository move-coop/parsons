...
def main():
    ...
    if limited_deps.strip().upper() in ("1", "YES", "TRUE", "ON"):
        ...
        extras_require = {
            ...
            "catalist": ["paramiko"],
            "civis": ["civis"],
            "dbt-redshift": ["dbt-redshift", "slackclient<3"],
            "dbt-bigquery": ["dbt-bigquery", "slackclient<3"],
            "dbt-postgres": ["dbt-postgres", "slackclient<3"],
            "dbt-snowflake": ["dbt-snowflake", "slackclient<3"],
