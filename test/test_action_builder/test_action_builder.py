...
class TestActionBuilder(unittest.TestCase):
    ...
    def setUp(self, m):
        ...
        self.fake_tags_list = (self.fake_tags_list_1['_embedded']['osdi:tags']
                                 + self.fake_tags_list_2['_embedded']['osdi:tags'])

        self.fake_tag_names = ['Fake Tag 5', self.fake_tag_4, self.fake_tag_3, self.fake_tag_2]
        ...
        self.fake_insert_person = {
            ...
            'data': {
                'person': {
                    'given_name': 'Fakey',
                    'family_name': 'McFakerson',
                    'email_address': [{'address': 'fakey@mcfakerson.com',
                                       'status': 'unsubscribed'}],
                    'created_date': self.fake_datetime,
                    'modified_date': self.fake_datetime
                }
        ...
        self.fake_update_person = {k:v for k,v in self.fake_insert_person.items()
                                   if k != 'entity_type'}
        self.fake_update_person['identifiers'] = [f'action_builder:{self.fake_entity_id}']

        self.fake_connection = {'person_id': 'fake-entity-id-2'}

    @requests_mock.Mocker()
    def test_get_page(self, m):
        m.get(f"{self.api_url}/tags?page=2&per_page=2", text=json.dumps(self.fake_tags_list_2))
        self.assertEqual(self.bldr._get_page(self.campaign, 'tags', 2, 2), self.fake_tags_list_2)

    @requests_mock.Mocker()
    def test_get_entry_list(self, m):
        m.get(f"{self.api_url}/tags?page=1&per_page=25", text=json.dumps(self.fake_tags_list_1))
        m.get(f"{self.api_url}/tags?page=2&per_page=25", text=json.dumps(self.fake_tags_list_2))
        m.get(f"{self.api_url}/tags?page=3&per_page=25",
              text=json.dumps({'_embedded': {"osdi:tags": []}}))
        assert_matching_tables(self.bldr._get_entry_list(self.campaign, 'tags'),
    ...
    def test_get_tag_by_name(self, m):
        m.get(f"{self.api_url}/tags?filter=name eq '{self.fake_tag_1}'",
            text=json.dumps(self.fake_tag_name_search_result))
        m.get(f"{self.api_url}/tags?page=2&per_page=25&filter=name eq '{self.fake_tag_1}'",
              text=json.dumps({'_embedded': {"osdi:tags": []}}))
        assert_matching_tables(self.bldr.get_tag_by_name(self.fake_tag_1),
    ...
    def test_upsert_entity(self, m):
        ...
        insert_flattened = {**{k:v for k,v in self.fake_insert_person.items() if k != 'data'},
                            **self.fake_insert_person['data']['person']}
        insert_incoming, insert_compare = self.compare_to_incoming(
            self.bldr.upsert_entity(**self.fake_insert_person),
            insert_flattened
        )

        update_flattened = {**{k:v for k,v in self.fake_update_person.items() if k != 'data'},
                             **self.fake_update_person['data']['person']}
        update_incoming, update_compare = self.compare_to_incoming(
            self.bldr.upsert_entity(**self.fake_update_person),
            update_flattened
        )
