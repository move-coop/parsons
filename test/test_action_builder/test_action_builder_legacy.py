"""Pre-migration ActionBuilder tests, kept during the parallel-change bake.

This is the original suite, marked ``legacy`` so it runs alongside the migrated
``test_action_builder.py``. Delete it at the "contract" step once the new suite
has baked (see test/MIGRATION.md); do not add to it.
"""
