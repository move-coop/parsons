...
class TestShopify(unittest.TestCase):
    ...
    def test_get_count(self, m):
        ...
        m.get(self.shopify.get_query_url('2020-10-20', None, "orders", True), json=self.mock_count_date)
    ...
    def test_get_orders(self, m):
        ...
        m.get(self.shopify.get_query_url('2020-10-20', None, 'orders', False), json=self.mock_orders_date)
        m.get(self.shopify.get_query_url(None, 2, 'orders', False), json=self.mock_orders_since)
        m.get(self.shopify.get_query_url(None, None, 'orders', False) + '&financial_status=paid', json=self.mock_orders_completed)
    ...
    def test_get_query_url(self, m):
        self.assertEqual(self.shopify.get_query_url(None, None, "orders", True), f'https://{SUBDOMAIN}.myshopify.com/admin/api/{API_VERSION}/orders/count.json?limit=250&status=any')
        self.assertEqual(self.shopify.get_query_url('2020-10-20', None, "orders", True), f'https://{SUBDOMAIN}.myshopify.com/admin/api/{API_VERSION}/orders/count.json?limit=250&status=any&created_at_min=2020-10-20T00:00:00&created_at_max=2020-10-21T00:00:00')
        self.assertEqual(self.shopify.get_query_url(None, 2, "orders", True), f'https://{SUBDOMAIN}.myshopify.com/admin/api/{API_VERSION}/orders/count.json?limit=250&status=any&since_id=2')
        self.assertEqual(self.shopify.get_query_url(None, None, "orders", False), f'https://{SUBDOMAIN}.myshopify.com/admin/api/{API_VERSION}/orders.json?limit=250&status=any')