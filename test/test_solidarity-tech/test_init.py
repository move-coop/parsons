"""Tests for initializing :class:`~parsons.solidarity_tech.SolidarityTech`."""
...
def test_init_with_arg() -> None:
    ...
    assert isinstance(st.api.session.auth, BearerAuth)

    req = requests.Request("GET", url="https://api.example.com", auth=st.api.session.auth)
    req = req.prepare()
