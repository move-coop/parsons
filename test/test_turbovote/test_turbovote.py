...
class TestTurboVote(unittest.TestCase):
    ...
    def test_get_users(self, m):
        ...
        expected = ['id', 'first-name', 'middle-name', 'last-name',
                    'phone', 'email', 'registered-address-street',
                    'registered-address-street-2', 'registered-address-city',
                    'registered-address-state', 'registered-address-zip',
                    'mailing-address-street', 'mailing-address-street-2',
                    'mailing-address-city', 'mailing-address-state',
                    'mailing-address-zip', 'dob', 'language-preference',
                    'hostname', 'referral-code', 'partner-comms-opt-in',
                    'created-at', 'updated-at', 'voter-registration-status',
                    'voter-registration-source', 'voter-registration-method',
                    'voting-method-preference', 'email subscribed',
                    'sms subscribed']
        ...
        with open(f'{_dir}/users.txt', 'r') as users_text:
            ...
            # Mock endpoints
            m.post(self.tv.uri + 'login', json=fake_token)
            m.get(self.tv.uri + f'partners/{self.tv.subdomain}.turbovote.org/users',
