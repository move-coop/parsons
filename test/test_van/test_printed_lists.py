...
class TestSavedLists(unittest.TestCase):
    ...
    def test_live_printed_lists(self):
        result = self.van.get_printed_lists(folder_name = 'Covington Canvass Turfs')
        print(result)

    def test_get_printed_list(self):
        result = self.van.get_printed_list(printed_list_number = '38988743-33091')
        print(result)

    def test_use_printed_saved(self):
        result = self.van.get_saved_list(saved_list_id = '38988743-33091')
        print(result)

    # @requests_mock.Mocker()
    # def test_get_saved_lists(self, m):

    #     json = {'folderame': 'Covington Canvass Turfs'}

    #     m.get(self.van.connection.uri + 'printedLists', json=json)

    #     expected = []

    #     print('expecting result')
    #     print(self.van.get_printed_lists())

    #     self.assertTrue(validate_list(expected, self.van.get_printed_lists()))
