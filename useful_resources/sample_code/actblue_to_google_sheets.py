...
config_vars = {
    ...
    # Connector 2:
    "GOOGLE_DRIVE_CREDENTIALS": "" # visit https://console.developers.google.com to create a Service Account and download its credentials in a json file. Provide the path to that file for this configuration.
}
...
from parsons.actblue import ActBlue
...
# A new spreadsheet with this name will be created, even if another spreadsheet with this name already exists.
# The Google Sheets connector will create the new spreadsheet, with 0 rows, and then overwrite the empty spreadsheet
# with your data. This script will not overwrite or update existing Google Sheets.
spreadsheet_name = 'Contributions - January'

# This script will create a Google Sheet with Restriced permissions -- only people specifically added can open it.
# The account specifed in the Google credentials file will be the owner of the new spreadsheet.
# If you created a Service Account, the Service Account will be the owner.
# In this case, to receive a link to the spreadsheet and editor permissions, you must provide your email address.
# Once the sheet has been created you may add user permissions in Google Sheets.
editor_email = ''
if not editor_email:
