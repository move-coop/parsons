...
def get_info_from_van(van_folder):
    ...
    saved_lists_raw = van.get_saved_lists(
        folder_id=folder_id).to_dataframe().drop_duplicates(subset=['name'])
    ...
    saved_printed_merged_raw = saved_lists[[
        'name', 'listCount', 'doorCount']].merge(printed_lists, on='name')
    saved_printed_merged_renamed = saved_printed_merged_raw.rename(
        columns={'name': 'Turf Name', 'listCount': 'Person Count', 'doorCount': 'Door Count', 'number': 'List Number'})
