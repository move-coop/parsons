...
def get_info_from_van(van_folder):
    ...
    printed_lists = van.get_printed_lists(folder_name=van_folder).to_dataframe()[
        ["number", "name"]
    ]
    ...
    saved_lists_raw = (
        van.get_saved_lists(folder_id=folder_id)
        .to_dataframe()
...
def to_gsheet(saved_printed_merged, gsheet_uri):
    ...
    sheets.overwrite_sheet(
        gsheet_uri, Table.from_dataframe(saved_printed_merged), worksheet="all"
    )
